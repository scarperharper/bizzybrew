...
export function ReceiptSummaryPanel({
  ...
  const dataFilter = (data: TransformedReceiptSummary) => {
    return [data.name, ...data.groups.flatMap(g => g.purchases.map(p => p.stock_line.name))].some(item => item.toLowerCase().includes(filter.toLowerCase()))
  }
  ...
  const data = receiptSummary.map(summary => ({
    ...
    date: summary.date,
    stats: summary.stock_purchase.reduce((stats, purchase) => {
    ...
    groups: summary.stock_purchase.reduce((groups, purchase) => {
          ...
          <div className="grow justify-self-stretch">
            <table className="table-auto w-80 justify-self-end max-w-64 ml-auto">
              ...
              {receipt.groups.map((group) => (
                <tr key={group.id} className="text-left leading-4">
                  <th className="text-xs font-mono">{group.name}</th>
                  <td className="flex flex-row">
                    <div
                      ...
                      title={group.purchases.map(p => p.stock_line.name).join(", ")}>&nbsp;</div>
                  </td>
                </tr>
              ))}
            </table>
